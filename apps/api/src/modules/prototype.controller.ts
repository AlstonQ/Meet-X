...
import { addPrototypeNote, deletePrototypeMeeting, getPrototypeMeeting, getRecordingStream, listPrototypeMeetings, saveUploadedMeeting, updatePrototypeMeeting, type PrototypeMeeting, type PrototypeNote } from "./prototype-store.js";
...
export class PrototypeController {
  ...
  async recording(@Param("id") id: string, @Res({ passthrough: true }) response: ServerResponse): Promise<unknown> {
    const stream = await getRecordingStream(id);
    if (stream === undefined) throw new HttpException("Recording not found", HttpStatus.NOT_FOUND);
    response.setHeader("Content-Type", stream.mimeType);
    return stream.file;
  }
